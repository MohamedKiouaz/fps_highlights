...
def get_image_similarity(image_array):
    return np.array([[np.linalg.norm(image_array[i] - image_array[j]) for j in range(len(image_array))] for i in range(len(image_array))])
...
if __name__ == '__main__':
    ...
    for folder in folders:
        ...
        for j in range(image_similarity.shape[0]):
            ...
            if(image_similarity[j][most_similar[0]] > 10000 or image_similarity[j][most_similar[0]] < 1):
                continue
            ...
            axs[i, 0].set_title(f'{image_similarity[j][most_similar[0]]:.2f} {image_list[j]}')
            axs[i, 1].set_title(f'{image_similarity[j][most_similar[1]]:.2f} {image_list[most_similar[0]]}')
