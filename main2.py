...
class VideoProcessor:
    ...
    def routine(self):
        for filename in os.listdir(folder):
            ...
            mask = self.shorten(vfc, filename)
            if not self.generate_inputs:
                self.generate_video(vfc, mask, filename)

    def shorten(self, clip, filename):
        ...
        if clip.duration <= self.predict_sampling / clip.fps:
            ...
            return self.process(clip, filename)
        
        mid = clip.duration / 2

        if self.multi_thread and self.executor._work_queue.empty():
            m1 = self.executor.submit(self.shorten, clip.subclip(t_start=0, t_end=mid), filename)
            m2 = self.executor.submit(self.shorten, clip.subclip(t_start=mid, t_end=clip.duration), filename)
            m1 = m1.result()
            m2 = m2.result()
        else:
            m1 = self.shorten(clip.subclip(t_start=0, t_end=mid), filename)
            m2 = self.shorten(clip.subclip(t_start=mid, t_end=clip.duration), filename)

        return np.concatenate((m1, m2))

    def process(self, clip, filename):
        subframes = {}
